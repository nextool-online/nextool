...
export default function SavingsCalculatorTool({ lang, ui }: ToolComponentProps) {
  ...
  return <ToolBox className={shellClass}><ToolSection title={getText(toolUi.heading, lang)} description={getText(toolUi.helper, lang)}><div className="space-y-6"><div className="grid gap-3 md:grid-cols-4"><MoneyInput value={initialDeposit} setValue={setInitialDeposit} placeholder={getText(toolUi.initialDeposit, lang)} /><MoneyInput value={monthlySavings} setValue={setMonthlySavings} placeholder={getText(toolUi.monthlySavings, lang)} /><MoneyInput value={interestRate} setValue={setInterestRate} placeholder={getText(toolUi.interestRate, lang)} /><MoneyInput value={years} setValue={setYears} placeholder={getText(toolUi.years, lang)} /></div><MoneyResultCard label={getText(toolUi.futureValue, lang)} value={result ? formatCurrency(result.futureValue, lang) : "—"} helper={helper} />{result && <MoneyMetricGrid metrics={[{ label: getText(toolUi.totalDeposited, lang), value: formatCurrency(result.totalDeposited, lang) }, { label: getText(toolUi.interestEarned, lang), value: formatCurrency(result.interestEarned, lang) }, { label: lang === "pt" ? "Juros sobre depósitos" : "Interest on deposits", value: formatPercent(result.totalDeposited ? (result.interestEarned / result.totalDeposited) * 100 : 0, lang) }]} />}{result && <MoneyContextualNextSteps lang={lang} title={lang === "pt" ? "Transforme esta meta em uma jornada" : "Turn this goal into a journey"} description={lang === "pt" ? "Depois de estimar a poupança, compare crescimento, aposentadoria e inflação para ver se a meta continua realista." : "After estimating savings, compare growth, retirement and inflation to see whether the goal still holds."} steps={[{ label: lang === "pt" ? "Projetar investimento" : "Project investment", href: lang === "pt" ? "/tools/calculadora-investimentos" : "/tools/investment-calculator", helper: lang === "pt" ? "Veja se aportes e retorno podem acelerar a meta." : "See whether contributions and returns can accelerate the goal." }, { label: lang === "pt" ? "Planejar aposentadoria" : "Plan retirement", href: lang === "pt" ? "/tools/calculadora-aposentadoria" : "/tools/retirement-calculator", helper: lang === "pt" ? "Conecte economia mensal a uma meta futura maior." : "Connect monthly savings to a larger future target." }, { label: lang === "pt" ? "Ajustar por inflação" : "Adjust for inflation", href: lang === "pt" ? "/tools/calculadora-inflacao" : "/tools/inflation-calculator", helper: lang === "pt" ? "Confira se o poder de compra acompanha a meta." : "Check whether purchasing power keeps up with the goal." }]} />}<MoneyToolCallout lang={lang} /></div></ToolSection></ToolBox>;
}
