...
export default function CalorieCalculatorTool({ lang, ui }: ToolComponentProps) {
  const toolUi = ui!;
  ...
  const result = useMemo(() => {
    ...
    return {
      ...
      lose,
      gain,
      marker: getCalorieMarker(maintenance),
    };
            ...
            >
              {activityLevels.map((level) => (
                <option key={level} value={level}>
                  {getText(toolUi[level], lang)} · {activityMultipliers[level]}x
                </option>
              ))}
            </ToolSelect>
          </div>
              ...
              <div className="grid gap-4 md:grid-cols-3">
                ...
                <div className="rounded-2xl border border-sky-200 bg-sky-50 p-4 text-center">
                  ...
                  <p className="mt-2 text-2xl font-black text-sky-700">
                    {formatNumber(result.gain, lang, { maximumFractionDigits: 0 })} <span className="text-sm font-black text-sky-700/70">{calorieUnit}</span>
                  </p>
                </div>
              </div>
