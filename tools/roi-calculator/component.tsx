...
export default function RoiCalculatorTool({ lang, ui }: ToolComponentProps) {
  ...
  return <ToolBox className={shellClass}><ToolSection title={getText(toolUi.heading, lang)} description={getText(toolUi.helper, lang)}><div className="space-y-6"><div className="grid gap-3 md:grid-cols-2"><MoneyInput value={investmentCost} setValue={setInvestmentCost} placeholder={getText(toolUi.investmentCost, lang)} /><MoneyInput value={netProfit} setValue={setNetProfit} placeholder={getText(toolUi.netProfit, lang)} /></div><MoneyResultCard label={getText(toolUi.roi, lang)} value={result ? `${formatNumber(result.roi, lang)}%` : "—"} helper={helper} tone={result && result.roi < 0 ? "amber" : "emerald"} />{result && <MoneyMetricGrid metrics={[{ label: getText(toolUi.totalReturn, lang), value: formatCurrency(result.totalReturn, lang) }, { label: lang === "pt" ? "Lucro líquido" : "Net profit", value: formatCurrency(result.profit, lang) }, { label: lang === "pt" ? "Custo" : "Cost", value: formatCurrency(result.cost, lang) }]} />}{result && <MoneyContextualNextSteps lang={lang} title={lang === "pt" ? "Transforme o ROI em decisão operacional" : "Turn ROI into an operating decision"} description={lang === "pt" ? "Depois de medir retorno, confira ponto de equilíbrio, margem e próximos cenários antes de escalar." : "After measuring return, check break-even, margin and next scenarios before scaling."} steps={[{ label: lang === "pt" ? "Encontrar ponto de equilíbrio" : "Find break-even", href: lang === "pt" ? "/tools/calculadora-ponto-de-equilibrio" : "/tools/break-even-calculator", helper: lang === "pt" ? "Veja quantas vendas cobrem os custos." : "See how many sales cover the costs." }, { label: lang === "pt" ? "Calcular porcentagem" : "Calculate percentage", href: lang === "pt" ? "/tools/calculadora-porcentagem" : "/tools/percentage-calculator", helper: lang === "pt" ? "Use para margem, taxa, desconto ou variação." : "Use it for margin, fee, discount or change." }, { label: lang === "pt" ? "Voltar ao Money" : "Open Money hub", href: "/money", helper: lang === "pt" ? "Compare outra decisão financeira por intenção." : "Compare another financial decision by intent." }]} />}<MoneyToolCallout lang={lang} /></div></ToolSection></ToolBox>;
}
